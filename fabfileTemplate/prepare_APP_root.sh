#!/bin/bash
#
# Utility script to create a new APP root
#
...
function print_usage {
	echo "Creates and prepares a new APP root directory"
	echo
	echo "$0 [-h | -?] [-f] [-D] [-C] <APP root directory>"
	echo
	echo "-h, -?: Show this help"
	echo "-f: Force creation of APP root, even if directory exists"
...
cfg_file="${root_dir}/cfg/appServer.json"
db_file="${root_dir}/APP.sqlite"

# Create and populate the root directory
echo "Creating APP root directory"
mkdir -p "${root_dir}" || error "Failed to create APP root directory"
cp -R APP/* "${root_dir}" || error "Failed to populate APP root with initial contents"

# Copy sample configuration file and adjust it to use an sqlite3 database
# located in the APP root directory (if required)
if [ "${CREATE_CFG}" = "yes" ]
then
...
echo
echo "----------------------------------------------------------------------------"
echo "Successfully setup ${root_dir} as an APP root directory"
echo
if [ "${CREATE_CFG}" = "yes" ]
then
...
echo "To try out your new APP root"
echo
echo "----------------------------------------------------------------------------"
echo
